 ...
 * @LastEditTime: 2023-10-15 00:47:38
...
#include <fstream>
#include <string.h>
#include "../include/gilib_interface.h"
...
int main(int argc, char** argv) {
    ...
    while (!feof(fpin_imu))
    {
        if (imu.time_stamp < gpos.time_stamp)
        {
            ...
            ret = gilib.gins_process(prctype_imu, &imu, sizeof(imu_t), NULL, 0);
            decode_imu(fpin_imu, imu);
        }
        else
        {
            ...
            ret = gilib.gins_process(prctype_gpos, &gpos, sizeof(gpos_t), NULL, 0);
            decode_gpos(fpin_gnss, gpos);
        }
    }
